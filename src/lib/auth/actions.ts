...
import { registerSchema, loginSchema, forgotPasswordSchema, resetPasswordSchema } from "@/lib/auth/validation";
...
async function sendVerificationEmail(email: string, token: string): Promise<ActionResult> {
  ...
  if (!delivery.delivered) {
    return { ok: false, error: "We couldn't send the verification email. Please try again shortly." };
  }
  return { ok: true };
}

async function issueVerificationToken(userId: string): Promise<{ token: string }> {
  const { token, tokenHash } = generateSecureToken();
  await db.$transaction([
    db.emailVerificationToken.updateMany({ where: { userId, usedAt: null }, data: { usedAt: new Date() } }),
...
export async function registerAction(input: unknown): Promise<ActionResult> {
  ...
  const participantRole = await db.role.findUnique({ where: { key: "PARTICIPANT" } });
  if (!participantRole) {
    console.error("[auth] Registration unavailable: PARTICIPANT role has not been seeded.");
  ...
  try {
    const user = await db.$transaction(async (tx) => {
      ...
      await tx.userRole.create({ data: { userId: created.id, roleId: participantRole.id, assignedBy: null } });
      await tx.emailVerificationToken.create({
        data: { userId: created.id, tokenHash, expiresAt: new Date(Date.now() + EMAIL_VERIFICATION_TTL_MS) },
      });
      return created;
    });
    userId = user.id;
    ...
    console.error("[auth] Registration database operation failed.", { message: error instanceof Error ? error.message : "unknown" });
    return { ok: false, error: "We couldn't create the account. Please try again." };
  }

  const delivery = await sendVerificationEmail(email, token);
  if (!delivery.ok) return delivery;

  await recordAudit({ actorId: userId, action: "auth:register", targetType: "user", targetId: userId });
...
export async function loginAction(input: unknown): Promise<ActionResult> {
  ...
  if (!user) {
    await recordAudit({ actorId: null, action: "auth:login_failed", metadata: { reason: "no_such_user" } });
    return { ok: false, error: "Incorrect email or password." };
  }

  const validPassword = await verifyPassword(password, user.passwordHash);
  if (!validPassword) {
    await recordAudit({ actorId: user.id, action: "auth:login_failed", metadata: { reason: "bad_password" } });
    return { ok: false, error: "Incorrect email or password." };
  }
  ...
  await recordAudit({ actorId: user.id, action: "auth:login", targetType: "user", targetId: user.id });
...
export async function logoutAction(): Promise<void> {
  ...
  if (session) {
    await recordAudit({ actorId: session.id, action: "auth:logout", targetType: "user", targetId: session.id });
  }
...
export async function verifyEmailAction(token: string): Promise<ActionResult> {
  ...
  if (!record) return { ok: false, error: "This verification link is invalid or has expired." };
  ...
  if (!consumed) return { ok: false, error: "This verification link is invalid or has expired." };

  await recordAudit({ actorId: record.userId, action: "auth:email_verified", targetType: "user", targetId: record.userId });
...
export async function forgotPasswordAction(input: unknown): Promise<ActionResult> {
  ...
  if (user) {
    ...
    await db.$transaction([
      ...
      db.passwordResetToken.create({ data: { userId: user.id, tokenHash, expiresAt: new Date(Date.now() + PASSWORD_RESET_TTL_MS) } }),
    ]);

    const delivery = await sendEmail({
      to: email,
      subject: "Reset your ACOB password",
      html: passwordResetEmailHtml(`${getSiteUrl()}/reset-password?token=${token}`),
    });
    if (!delivery.delivered) console.error("[auth] Password reset email could not be delivered.", { userId: user.id });

    await recordAudit({ actorId: user.id, action: "auth:password_reset_requested", targetType: "user", targetId: user.id });
  }
...
export async function resetPasswordAction(input: unknown): Promise<ActionResult> {
  ...
  if (!consumed) return { ok: false, error: "This reset link is invalid or has expired." };
...
export async function resendVerificationAction(): Promise<ActionResult> {
  ...
  const rl = rateLimit(`resend-verification:${session.id}`, { limit: 3, windowMs: 60 * 60 * 1000 });
  if (!rl.allowed) {
    return { ok: false, error: "Too many attempts. Try again later." };
  }
